...
public class SoundFile {
    ...
    public class DurationException extends Exception {
        // Serial version ID generated by Eclipse.
        private static final long serialVersionUID = -2505698991567443445L;
    ...
    public static SoundFile create(String fileName,
        ...
        throws java.io.FileNotFoundException,
               IOException, InvalidInputException,DurationException {
...
//    }
    ...
    private void ReadFile(File inputFile)
        ...
        if(duration > 600000000){
            throw new DurationException("时间不能大于10分钟");
        }
        ...
        while(true) {
            try {
                mDecodedBytes = ByteBuffer.allocateDirect(1<<20);
                break;
...
//                    int retry = 10;
                    while(true) {
                        try {
                            newDecodedBytes = ByteBuffer.allocateDirect(newSize);
                            break;
                        } catch (OutOfMemoryError oome) {
                            System.gc();
                        }
...
//                    }
                ...
                if (mProgressListener != null) {
                    if (!mProgressListener.reportProgress((float)(info.presentationTimeUs) / duration)) {
        ...
        codec = null;
        if (mProgressListener != null) {
            if (!mProgressListener.reportProgress(1)) {
                return;
            }
        }
...
//        mDecodedBytes = ByteBuffer.allocateDirect(20 * mSampleRate * 2);
//        mDecodedBytes.order(ByteOrder.LITTLE_ENDIAN);
//        mDecodedSamples = mDecodedBytes.asShortBuffer();
...
//        ByteBuffer encodedBytes = ByteBuffer.allocateDirect(estimatedEncodedSize);
//        ByteBuffer[] inputBuffers = codec.getInputBuffers();
//        ByteBuffer[] outputBuffers = codec.getOutputBuffers();
//        MediaCodec.BufferInfo info = new MediaCodec.BufferInfo();
//        boolean done_reading = false;
...
//                    ByteBuffer newEncodedBytes = ByteBuffer.allocateDirect(estimatedEncodedSize);
